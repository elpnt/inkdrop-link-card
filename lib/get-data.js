...
function getImage(url, dom) {
  ...
  if (ogImage) image = ogImage.getAttribute("content").split("?")[0]; // If og:image is relative path, convert it to absolute one
...
function getDescription(dom) {
  let description = "";
  const ogDescription = dom.head.querySelector("meta[property='og:description']");
